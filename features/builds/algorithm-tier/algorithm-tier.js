/**
 * Lógica de la feature Builds/Algorithm Tier List
 * Maneja la UI, modal, localStorage y renderizado de builds
 * Prioriza componentes de gama alta sin considerar precio
 */

import { calculateScore, getCategory } from './scoring-engine-tier.js';
...
const builds = [];
const STORAGE_KEY = 'pcGamerBuildsTier_v1';
...
function renderTable() {
  ...
  builds.forEach((build, index) => {
    ...
    const gamingClass = build.gaming === 'Extrema' ? 'good' : build.gaming === 'Excelente' ? 'good' : 'mid';
    const iaClass = build.ia === 'Extrema' ? 'good' : build.ia === 'Excelente' ? 'good' : build.ia === 'Muy buena' ? 'mid' : '';
    const futureClass = build.future === 'Extrema' ? 'good' : build.future === 'Muy alta' ? 'good' : build.future === 'Alta' ? 'mid' : '';
